...
class CustomCosyVoice:
    ...
    def inference_sft(self, tts_text, spk_id):
        ...
        if len(tts_speeches) == 0:
            raise RuntimeError(f"No TTS speech generated in inference_sft. tts_text={repr(tts_text)}")
    ...
    def inference_zero_shot(self, tts_text, prompt_text, prompt_speech_16k):
        ...
        if len(tts_speeches) == 0:
            raise RuntimeError(
                "No TTS speech generated in inference_zero_shot.\n"
                f"tts_text={repr(tts_text)}\n"
                f"prompt_text={repr(prompt_text)}"
            )
    ...
    def inference_zero_shot_no_unit_condition_no_normalize(
        ...
        if tts_text == "":
            raise ValueError("tts_text is empty after strip().")

        if prompt_text == "":
            raise ValueError("prompt_text is empty after strip().")

        tts_speeches = []
        pieces = re.split(r"(?<=[？！。.?!])\s*", tts_text)

        print("[Debug] Split tts_text pieces:", pieces)
        ...
        for i in pieces:
            ...
            if "tts_speech" not in model_output or model_output["tts_speech"] is None:
                print(f"[Warning] Empty model output for piece: {repr(i)}")
                continue
        ...
        if len(tts_speeches) == 0:
            raise RuntimeError(
                "No TTS speech generated in inference_zero_shot_no_unit_condition_no_normalize.\n"
                f"Original tts_text={repr(tts_text)}\n"
                f"Split pieces={repr(pieces)}\n"
                f"prompt_text={repr(prompt_text)}"
            )
    ...
    def inference_zero_shot_no_normalize(self, tts_text, prompt_text, prompt_speech_16k):
        ...
        for i in pieces:
            ...
            try:
                ...
                if "tts_speech" not in model_output or model_output["tts_speech"] is None:
                    print(f"[Warning] Model returned no tts_speech for piece: {repr(i)}")
                    continue
                ...
                raise RuntimeError(
                    "TTS failed while processing one text piece.\n"
                    f"piece={repr(i)}\n"
                    f"full_tts_text={repr(tts_text)}\n"
                    f"prompt_text={repr(prompt_text)}\n"
                    f"error={repr(e)}"
                ) from e
        ...
        if len(tts_speeches) == 0:
            raise RuntimeError(
                "No TTS speech generated in inference_zero_shot_no_normalize.\n"
                f"Original tts_text={repr(tts_text)}\n"
                f"Split pieces={repr(pieces)}\n"
                f"prompt_text={repr(prompt_text)}\n"
                "This usually means the input text became empty/invalid after preprocessing, "
                "or every split piece was skipped."
            )
...
def get_bopomofo_rare(text, converter):
    ...
    for i in range(len(text_w_bopomofo)):
        ...
        try:
            freq = word_to_dataset_frequency[t[0]]
        except Exception:
            freq = 999999999
        ...
        if freq < 500 and t[1] is not None and next_t_char != "[":
            reconstructed_text += t[0] + f"[:{t[1]}]"

        elif len(phns) >= 2:
            if t[1] != phns[0] and (freq < 10000 or t[0] in always_augment_chars) and next_t_char != "[":
                reconstructed_text += t[0] + f"[:{t[1]}]"
            else:
...
def single_inference(
    ...
    speaker_prompt_audio_path = str(speaker_prompt_audio_path).strip()
    output_path = str(output_path).strip()

    if not os.path.exists(speaker_prompt_audio_path):
        raise FileNotFoundError(f"Prompt audio not found: {speaker_prompt_audio_path}")

    if content_to_synthesize is None or str(content_to_synthesize).strip() == "":
        raise ValueError(
            "content_to_synthesize is empty before preprocessing.\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}"
        )

    prompt_speech_16k = load_wav(speaker_prompt_audio_path, 16000)

    content_to_synthesize = str(content_to_synthesize).strip()

    if speaker_prompt_text_transcription is not None and str(speaker_prompt_text_transcription).strip() != "":
        speaker_prompt_text_transcription = str(speaker_prompt_text_transcription).strip()
    else:
        speaker_prompt_text_transcription = transcribe_audio(speaker_prompt_audio_path)

    if speaker_prompt_text_transcription is None or str(speaker_prompt_text_transcription).strip() == "":
        raise ValueError(
            "speaker_prompt_text_transcription is empty before preprocessing.\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"content_to_synthesize={repr(content_to_synthesize)}\n"
            f"output_path={output_path}"
        )
    ...
    try:
        ...
        speaker_prompt_text_transcription = converter.convert(speaker_prompt_text_transcription_norm)

        content_to_synthesize_norm = cosyvoice.frontend.text_normalize_new(
            content_to_synthesize,
            split=False,
        )
        content_to_synthesize = converter.convert(content_to_synthesize_norm)

    except Exception as e:
        raise RuntimeError(
            "Text normalization failed.\n"
            f"speaker_prompt_text_transcription={repr(speaker_prompt_text_transcription)}\n"
            f"content_to_synthesize={repr(content_to_synthesize)}\n"
            f"error={repr(e)}"
        ) from e
    ...
    if speaker_prompt_text_transcription == "":
        raise ValueError(
            "speaker_prompt_text_transcription became empty after normalization.\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}"
        )

    if content_to_synthesize == "":
        raise ValueError(
            "content_to_synthesize became empty after normalization.\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Bopomofo conversion failed.\n"
            f"speaker_prompt_text_transcription={repr(speaker_prompt_text_transcription)}\n"
            f"content_to_synthesize={repr(content_to_synthesize)}\n"
            f"error={repr(e)}"
        ) from e

    if speaker_prompt_text_transcription_bopomo.strip() == "":
        raise ValueError(
            "speaker_prompt_text_transcription_bopomo is empty.\n"
            f"speaker_prompt_text_transcription={repr(speaker_prompt_text_transcription)}\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}"
        )

    if content_to_synthesize_bopomo.strip() == "":
        raise ValueError(
            "content_to_synthesize_bopomo is empty.\n"
            f"content_to_synthesize={repr(content_to_synthesize)}\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "cosyvoice.inference_zero_shot_no_normalize failed.\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}\n"
            f"raw_content_to_synthesize={repr(content_to_synthesize)}\n"
            f"content_to_synthesize_bopomo={repr(content_to_synthesize_bopomo)}\n"
            f"speaker_prompt_text_transcription_bopomo={repr(speaker_prompt_text_transcription_bopomo)}\n"
            f"error={repr(e)}"
        ) from e

    end = time.time()

    if output is None or "tts_speech" not in output or output["tts_speech"] is None:
        raise RuntimeError(
            "Model output is empty or missing tts_speech.\n"
            f"speaker_prompt_audio_path={speaker_prompt_audio_path}\n"
            f"output_path={output_path}\n"
            f"content_to_synthesize_bopomo={repr(content_to_synthesize_bopomo)}"
        )
...
def main():
    ...
    single_inference(
        args.speaker_prompt_audio_path,
        args.content_to_synthesize,
        args.output_path.strip(),
        cosyvoice,
        bopomofo_converter,
